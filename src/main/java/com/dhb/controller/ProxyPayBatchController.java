...
import com.dhb.service.ProxyPayBatchService;
import com.dhb.util.WebContextHolder;
import com.google.common.base.Strings;

@Controller
@RequestMapping("/proxyPayBatch")
public class ProxyPayBatchController {
	@Autowired
	private ProxyPayBatchService proxyPayBatchService;
	...
	public String list(ProxyBatchParam param, ModelMap model)
		...
		proxyPayBatchService.queryByForm(param);
		model.addAttribute("form", param);
		return "proxyPayBatch/list";
	}
	...
	public String verify(ProxyBatchParam param, Model model)
		...
		proxyPayBatchService.queryByForm(param);
		model.addAttribute("form", param);
		return "proxyPayBatch/verify";
	}

	@RequestMapping(value = "detail")
	public String detail(ProxyPayParam proxyPayParam, Model model) {
		List list = proxyPayBatchService
				.queryProxyPayBatchDetail(proxyPayParam.getBath_no());
		model.addAttribute("list", list);
		ProxyBatch proxyPayBatch = proxyPayBatchService.queryProxyBatchById(proxyPayParam.getBath_no());
		model.addAttribute("proxyBatch", proxyPayBatch);
		return "proxyPayBatch/detail";
	}
	...
	public String accountInfoImport(
		...
		proxyPayBatchService.importBatchFile(channelId,merchId, file);
		model.addAttribute("result", "上传成功！");
		return "proxyPayBatch/upload";
	}
	...
	public String audit(@Validated ProxyBatch proxyPayBatch, ModelMap model) throws ParseException,
		...
		try {
			proxyPayBatchService.audit(proxyPayBatch);
		} catch (Exception e) {
