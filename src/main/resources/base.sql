...
--===快捷支付sql===========2016-01-19==========================
--序列 用于生成银行订单号
 CREATE SEQUENCE  "POSP"."DHB_BANK_ORDERID_SEQ"  MINVALUE 1 MAXVALUE 999999 INCREMENT BY 1 START WITH 1 NOCACHE  NOORDER  CYCLE 

--快捷支付订单表
create table DHB_QUICKPAY_INFO  (
   ID                   VARCHAR2(36)                    not null,
   MERCHID              VARCHAR(20),
   MERORDERID           VARCHAR2(36),
   MERFEE               NUMBER(12,2)                   default 0,
   BIZTYPE              VARCHAR2(2),
   ACCNO                VARCHAR2(40),
   TXNAMT               NUMBER(12,2),
   PAYFLAG              VARCHAR2(2),
   CERTIFY_ID           VARCHAR2(40),
   CUSTOMERNM           VARCHAR2(40),
   PHONENO              VARCHAR2(20),
   EXPIRED              VARCHAR2(10),
   CVV2                 VARCHAR2(10),
   BACKURL              VARCHAR2(126),
   CREATEDATE           DATE,
   ISSETTLEMENT         VARCHAR2(2)                    default '0',
   STATE                NUMBER(12,2),
   MEMO                 VARCHAR2(126),
   constraint PK_DHB_QUICKPAY_INFO primary key (ID)
);

comment on table DHB_QUICKPAY_INFO is
'东汇宝快捷支付记录表';

comment on column DHB_QUICKPAY_INFO.MERCHID is
'商户号';

comment on column DHB_QUICKPAY_INFO.MERORDERID is
'商户订单号';

comment on column DHB_QUICKPAY_INFO.MERFEE is
'商户手续费';

comment on column DHB_QUICKPAY_INFO.BIZTYPE is
'业务类型：1快捷支付
';

comment on column DHB_QUICKPAY_INFO.ACCNO is
'银行卡号';

comment on column DHB_QUICKPAY_INFO.TXNAMT is
'交易金额 分';

comment on column DHB_QUICKPAY_INFO.PAYFLAG is
'支付方式1借记卡  2信用卡';

comment on column DHB_QUICKPAY_INFO.CERTIFY_ID is
'身份证号';

comment on column DHB_QUICKPAY_INFO.CUSTOMERNM is
'真实姓名';

comment on column DHB_QUICKPAY_INFO.PHONENO is
'手机号';

comment on column DHB_QUICKPAY_INFO.EXPIRED is
'信用卡有效期';

comment on column DHB_QUICKPAY_INFO.BACKURL is
'响应地址';

comment on column DHB_QUICKPAY_INFO.ISSETTLEMENT is
'是否结算
0未结算
1已结算';

comment on column DHB_QUICKPAY_INFO.STATE is
'交易状态0未完成  1成功  2失败';
 
--快捷支付交易表
create table DHB_QUICKPAY_JOUNAL  (
   ID                   VARCHAR2(36)                    not null,
   QUICKPAYID           VARCHAR2(36),
   BIZTYPE              VARCHAR2(2),
   MERFEE               NUMBER(12,2),
   BANKORDERID          VARCHAR2(36),
   BANKORDERAMT         NUMBER(12,2),
   STATE                VARCHAR2(2),
   BANKTN               VARCHAR2(36),
   BANKRETTIME          DATE,
   NETFEE               NUMBER(12,2),
   ...
   RESMSG               VARCHAR2(126),
   CREATETIME           DATE,
   BANKCHECK            VARCHAR2(2),
   SETTLEDATE           VARCHAR2(20),
   SETTLEAMT            VARCHAR2(12),
   constraint PK_DHB_QUICKPAY_JOUNAL primary key (ID)
);

comment on table DHB_QUICKPAY_JOUNAL is
'东汇宝快捷支付流水记录';

comment on column DHB_QUICKPAY_JOUNAL.BIZTYPE is
'业务类型 1快捷支付';

comment on column DHB_QUICKPAY_JOUNAL.MERFEE is
'商户手续费';

comment on column DHB_QUICKPAY_JOUNAL.BANKORDERID is
'银行订单号';

comment on column DHB_QUICKPAY_JOUNAL.BANKORDERAMT is
'订单金额 分';

comment on column DHB_QUICKPAY_JOUNAL.STATE is
'交易状态0未完成  1成功  2失败';

comment on column DHB_QUICKPAY_JOUNAL.BANKTN is
'银行返回流水';

comment on column DHB_QUICKPAY_JOUNAL.BANKRETTIME is
'银行返回时间';

comment on column DHB_QUICKPAY_JOUNAL.NETFEE is
'通道手续费';

comment on column DHB_QUICKPAY_JOUNAL.FEERATE is
'通道手续费率';

comment on column DHB_QUICKPAY_JOUNAL.CURRENCY is
'CNY';

comment on column DHB_QUICKPAY_JOUNAL.RESCODE is
'返回码';

comment on column DHB_QUICKPAY_JOUNAL.RESMSG is
'返回描述';

comment on column DHB_QUICKPAY_JOUNAL.BANKCHECK is
'是否对账0未对账  1已对账';

comment on column DHB_QUICKPAY_JOUNAL.SETTLEDATE is
'清算日期';

comment on column DHB_QUICKPAY_JOUNAL.SETTLEAMT is
'清算金额 分';
------------------------------------------------------
...
--==========手机充值sql============================
--通道订单号序列
CREATE SEQUENCE  "POSP"."DHB_OF_ORDERID_SEQ"  MINVALUE 1 MAXVALUE 999999 INCREMENT BY 1 START WITH 21 NOCACHE  NOORDER  CYCLE 

--订单表
create table DHB_MOBILE_ORDER  (
   ...
   ACCID                VARCHAR2(32),
   ACCTYPE              VARCHAR2(10),
   ACCAUTHMODE          VARCHAR2(10),
   ACQBIN               VARCHAR2(30),
   PDATE               VARCHAR(20),
   PTRACENUM            VARCHAR2(128),
   CURRENCY             VARCHAR(20),
   ORDERINFO            VARCHAR2(256),
   ORDERNUM             VARCHAR2(50),
   EXTINFO              VARCHAR2(256),
   PUBKEYINDEX          VARCHAR2(20),
   TICKET               VARCHAR2(256),
   EXT1                 VARCHAR2(256),
   MEMO                 VARCHAR2(256),
   constraint PK_DHB_MOBILE_ORDER primary key (ID)
);

comment on table DHB_MOBILE_ORDER is
'手机充值流水记录表';

comment on column DHB_MOBILE_ORDER.MERCHID is
'商户号';

comment on column DHB_MOBILE_ORDER.MOBILE is
'手机号';

comment on column DHB_MOBILE_ORDER.AMT is
'充值金额';

comment on column DHB_MOBILE_ORDER.CHANNELID is
'1.欧飞';

comment on column DHB_MOBILE_ORDER.PLATTIME is
'交易时间';

comment on column DHB_MOBILE_ORDER.STATE is
'支付状态，钱包消账  0初始化 1扣款成功  2失败  3撤销成功';

comment on column DHB_MOBILE_ORDER.ACCID is
'钱包id';

comment on column DHB_MOBILE_ORDER.ACCTYPE is
'钱包类型';

comment on column DHB_MOBILE_ORDER.ACCAUTHMODE is
'钱包授权模式';

comment on column DHB_MOBILE_ORDER.ACQBIN is
'受理方机构代码';

comment on column DHB_MOBILE_ORDER.PDATE is
'受理日期  yyyyMMddHHMMSS';

comment on column DHB_MOBILE_ORDER.PTRACENUM is
'受理方系统跟踪号';

comment on column DHB_MOBILE_ORDER.ORDERINFO is
'交易描述';

comment on column DHB_MOBILE_ORDER.ORDERNUM is
'订单号';

comment on column DHB_MOBILE_ORDER.EXTINFO is
'交易保留域';

comment on column DHB_MOBILE_ORDER.PUBKEYINDEX is
'公钥索引号';

comment on column DHB_MOBILE_ORDER.TICKET is
'账号签到信息';

comment on column DHB_MOBILE_ORDER.EXT1 is
'扩展字段';


--交易表
create table DHB_MOBILE_TRANSACTION  (
   ID                   VARCHAR2(40)                    not null,
   MOBORDERID           VARCHAR2(40),
   PLATTIME             DATE,
   AMT                  NUMBER(12,2),
   STATE                VARCHAR2(2),
   MOBILE               VARCHAR2(20),
   SPORDERID            VARCHAR2(40),
   OFCARDID             VARCHAR(20),
   OFCARDNUM            VARCHAR2(20),
   OFORDERCACHE         NUMBER(12,2),
   OFCARDNAME           VARCHAR2(256),
   OFTRACENUM           VARCHAR2(60),
   RETCODE              VARCHAR2(20),
   RETMSG               VARCHAR2(256),
   GAMESTATE            VARCHAR2(2),
   RETTIME              DATE,
   RETURL               VARCHAR2(128),
   EXT1                 VARCHAR2(256),
   constraint PK_DHB_MOBILE_TRANSACTION primary key (ID)
);

comment on table DHB_MOBILE_TRANSACTION is
'东汇宝交易表';

comment on column DHB_MOBILE_TRANSACTION.MOBORDERID is
'DHB_MOBILE_ORDER的id';

comment on column DHB_MOBILE_TRANSACTION.PLATTIME is
'交易时间';

comment on column DHB_MOBILE_TRANSACTION.AMT is
'金额 分';

comment on column DHB_MOBILE_TRANSACTION.STATE is
'充值状态0初始  1充值成功  2充值失败  3提交成功  4充值中';

comment on column DHB_MOBILE_TRANSACTION.MOBILE is
'手机号';

comment on column DHB_MOBILE_TRANSACTION.SPORDERID is
'商户传给欧飞的唯一编号';

comment on column DHB_MOBILE_TRANSACTION.OFCARDID is
'欧飞商品编号';

comment on column DHB_MOBILE_TRANSACTION.OFCARDNUM is
'欧飞商品数量';

comment on column DHB_MOBILE_TRANSACTION.OFORDERCACHE is
'欧飞实际充值金额';

comment on column DHB_MOBILE_TRANSACTION.OFCARDNAME is
'商品编号';

comment on column DHB_MOBILE_TRANSACTION.OFTRACENUM is
'欧飞订单流水号';

comment on column DHB_MOBILE_TRANSACTION.RETCODE is
'欧飞返回交易码';

comment on column DHB_MOBILE_TRANSACTION.RETMSG is
'交易描述';

comment on column DHB_MOBILE_TRANSACTION.GAMESTATE is
'澈消(充值失败)为9，充值中为0,只能当状态为9时，商户才可以退款给用户。';

comment on column DHB_MOBILE_TRANSACTION.EXT1 is
'扩展字段';
